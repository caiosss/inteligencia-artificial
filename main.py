...
for r in range(rodadas):
    ...
    for l in lambdas:
        ...
        beta_hat_ridge = pinv(X_treino.T@X_treino + l*I) @ X_treino.T @ y_treino
...
print("\n===== Resultados Médios =====")
print("Modelo da média -> MSE:", np.mean(resultados["media"]["mse"]), "R²:", np.mean(resultados["media"]["r2"]))
print("MQO sem intercepto -> MSE:", np.mean(resultados["mqo_sem_intercepto"]["mse"]), "R²:", np.mean(resultados["mqo_sem_intercepto"]["r2"]))
print("MQO com intercepto -> MSE:", np.mean(resultados["mqo_com_intercepto"]["mse"]), "R²:", np.mean(resultados["mqo_com_intercepto"]["r2"]))

for l in lambdas:
    print(f"Ridge (λ={l}) -> MSE:", np.mean(resultados["ridge"][l]["mse"]), "R²:", np.mean(resultados["ridge"][l]["r2"]))
